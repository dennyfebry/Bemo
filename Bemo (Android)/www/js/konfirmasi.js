...
function submitFor() {
    var temp = $('#konfirmasi-form').serialize();
    ...
    $.ajax({
        ...
        success: function (a) {
            console.log(a);
            $('#btn-konfirmasi').html('&nbsp; konfirmasi...').prop('disabled', true);
            $('input[type=text],input[type=date],input[type=time]').prop('disabled', true);
            if (a == 0) {
                $("#konfirmasi-form").trigger('reset');
                alert('Coba lagi...');
            } else {
                ...
                if (result.status === 'success') {
                    $('#errorDiv').slideDown('fast', function () {
                        ...
                        $('#btn-konfirmasi').html('<span class="glyphicon glyphicon-log-in"></span> &nbsp; konfirmasi').prop('disabled', false);
                    ...
                    setTimeout(function () {
                        window.location.href = 'tambah-servis.html?pesananID=' + pesanan_id
                    }, 3000)
                } else {
                    $('#errorDiv').slideDown('fast', function () {
